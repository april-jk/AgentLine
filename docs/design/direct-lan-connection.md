...
Wildcard DNS for a dedicated domain you control, for example `*.direct.example.com`, that embeds the target IP in the subdomain, similar to sslip.io:

```
192-168-1-5.direct.example.com  ->  192.168.1.5
10-0-0-42.direct.example.com    ->  10.0.0.42
```
...
A wildcard TLS cert for the direct-connection domain solves the mixed content problem. The cert exists purely to satisfy the browser's HTTPS requirement — actual security comes from SRP auth + NaCl encryption (same as relay connections).
...
**Security tradeoff**: Any authenticated server gets the wildcard private key, so a malicious user could theoretically impersonate any host under the direct-connection wildcard on the same LAN. This is acceptable because:
...
4. Client races two connections in parallel:
   ...
   b. Direct path: wss://192-168-1-5.direct.example.com:3400
...
- **IPv6**: Should the direct-connection wildcard also support IPv6 link-local addresses?
