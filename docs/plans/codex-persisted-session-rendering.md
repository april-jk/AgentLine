...
- Completed: Quality gates for this increment:
  - `pnpm --filter ./packages/server test -- test/sessions/codex-normalization.test.ts test/sessions/codex-reader-oss.test.ts`
  - `pnpm --filter ./packages/client test -- src/lib/__tests__/preprocessMessages.test.ts`
  - `pnpm lint`
  - `pnpm typecheck`
