...
function normalizeHttpBaseUrl(rawValue: string): string {
...
async function probeServer(baseUrl: string): Promise<LanScanResult | null> {
  try {
    ...
    const health = await withTimeout(
      fetchJson<{ status?: string }>(`${normalized}/health`),
      900,
    );
    if (health.status !== "ok") return null;

    const info = await withTimeout(
      fetchJson<ServerInfo>(`${normalized}/server-info`),
      1200,
    );
...
function dedupeUrls(urls: string[]): string[] {
  return Array.from(
    new Set(urls.map((url) => normalizeHttpBaseUrl(url)).filter(Boolean)),
  );
}
...
export async function scanLanServers(
  ...
  const results = await scanCandidates(
    buildSubnetCandidates(subnetPrefix, port),
  );
